"""
v004 — standalone webcam check for encoder_csn.pt.

NOT the shipping app. Does not import FastAPI or change backend/scripts.

    python backend/scriptsCSN/v004_live_csn.py
    python backend/scriptsCSN/v004_live_csn.py --vocab "HOLA,GRACIAS,ADIÓS"
"""
...
from backend.trainingCSN.paths import CK_CSN, LANG, REST_SIGN
...
def main():
    ...
    print(f"CSN head {len(labels)} classes, recorded val {val_acc}, {dev}")
    ...
    try:
        while True:
            ...
            view = cv2.flip(bgr, 1)
            cv2.putText(view, "LSC standalone — not integrated", (16, 28),
            ...
            cv2.imshow("LSC live (scriptsCSN)", view)
    finally:
        cap.release()
        extractor.close()
