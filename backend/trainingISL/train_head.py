"""
Train only the ISL head on a frozen GISLR encoder.

Reads encoder_ase.pt (does not modify it). Writes encoder_ins.pt.
...
from backend.trainingISL.dataset import (  # noqa: E402
    IsolatedSignDataset, load_label_list, load_rows, signer_independent_split,
)
from backend.trainingISL.paths import CK_ASE, CK_INS, LANG, OUTPUTS, REST_SIGN  # noqa: E402
...
def main():
    ...
    ap.add_argument("--val-signers", type=int, default=2)
    ...
    rows = load_rows()
    if rows.empty:
        sys.exit("no INCLUDE features — run backend.trainingISL.download_include")

    labels = load_label_list()
    # drop labels that never appear
    ...
    print(f"HELD-OUT PROXIES  {held}   <- MVI-bin signer split, not named people",
          flush=True)
    ...
    best, hist = 0.0, []
    CK_INS.parent.mkdir(parents=True, exist_ok=True)
    for ep in range(1, args.epochs + 1):
        ...
        if vaa >= best:
            ...
            torch.save({
                ...
                "note": "ISL head on frozen GISLR encoder. Not loaded by app/.",
            }, CK_INS)
            flag = "  <- saved"
    ...
    rep = OUTPUTS / "train_ins_history.json"
    ...
    print(f"weights  {CK_INS}\nhistory  {rep}")
    print("NOT wired into backend/app — load encoder_ins.pt later via add_head.")
