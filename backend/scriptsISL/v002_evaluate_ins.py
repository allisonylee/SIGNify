"""
v002 — evaluate encoder_ins.pt on held-out proxy signers. Does not touch app/.
...
from backend.trainingISL.dataset import IsolatedSignDataset, load_rows, signer_independent_split
from backend.trainingISL.paths import CK_INS, LANG, OUTPUTS


def main():
    if not CK_INS.exists():
        sys.exit(f"no {CK_INS} — run train_head first")
    ck = torch.load(CK_INS, map_location="cpu", weights_only=False)
    labels = ck["labels"]
    held = ck.get("held_out_signers") or []
    print(f"checkpoint {CK_INS.name}  classes {len(labels)}  recorded val {ck.get('val_acc')}")
    print(f"held-out proxies {held}")
    ...
    print(f"\nTOP-1 (proxy-signer-independent): {top1/n:.4f}  ({top1}/{n})")
